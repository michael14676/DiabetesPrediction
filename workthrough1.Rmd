...
# Part 2
for 6933. figure out the format of this stuff later.

## Logistic regression with new stuff
...
auc.list <- list()
# Print confusion matrix and auroc
for (i in seq_along(gam.metrics)) {
  # cat("=========== Fold", i, "===========\n")
  x <- gam.metrics[[i]]
  cm <- x[[1]]
  roc.obj <- x[[2]]
...
Going through the summary functions, each one agrees on Glucose, BMI, DiabetesPedigreeFunction, and Age having a significant smooth term. We will now try again with these four having nonlinear relationships and the other four having linear relationships:
...
auc.list <- list()
# Print confusion matrix and auroc
for (i in seq_along(mixed.metrics)) {
  # cat("=========== Fold", i, "===========\n")
  x <- mixed.metrics[[i]]
  cm <- x[[1]]
  roc.obj <- x[[2]]
...
auc.list <- list()
# Print confusion matrix and auroc
for (i in seq_along(interactions.metrics)) {
  # cat("=========== Fold", i, "===========\n")
  x <- interactions.metrics[[i]]
  cm <- x[[1]]
  roc.obj <- x[[2]]
  formula <- x[[3]]
  # print(cm)
...
spline.metrics <- lapply(fold.indices, spline.logistic)
```

Let's now look at the metrics:

```{r}
auc.list <- list()
# Print confusion matrix and auroc
for (i in seq_along(spline.metrics)) {
  # cat("=========== Fold", i, "===========\n")
  x <- spline.metrics[[i]]
  cm <- x[[1]]
  roc.obj <- x[[2]]
...
# par(mfrow=c(2,2))
# termplot(bs.model, se = TRUE, partial.resid = TRUE, rug = TRUE)
...
auc.list <- list()
# Print confusion matrix and auroc
for (i in seq_along(tree.metrics)) {
  # cat("=========== Fold", i, "===========\n")
  x <- tree.metrics[[i]]
  cm <- x[[1]]
  roc.obj <- x[[2]]
